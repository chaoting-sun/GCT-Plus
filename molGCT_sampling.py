...
=======
>>>>>>> 54a170d19337de0474ec43d922d34467dcb997fc:molGCT_sampling.py
import torch
import joblib
import argparse
...
=======
def inference(opt, model, SRC, TRG, robustScaler, toklen_data):
    # fix_random_seed()
    os.makedirs('molGCT/inference', exist_ok=True)
...
def inference_test(opt, model, SRC, TRG, robustScaler, toklen_data):
    logp = 3.0
    tpsa = 20.0
    qed = 0.6
    ...
    while True:
        ...
        if molecule is not None:
            logp_p, tpsa_p, qed_p = get_mol_prop(molecule)
            print(">>> SMILES:", smiles)
            print("    PROPS: {:.2f}\t{:.2f}\t{:.2f}".format(logp_p, tpsa_p, qed_p))
            number -= 1
        else:
            num_tried -= 1
        if number == 0 or num_tried == 0:
            break
...
=======
    ...
    inference_test(opt, model, SRC, TRG, robustScaler, toklen_data)

    # inference(opt, model, SRC, TRG, robustScaler, toklen_data)
>>>>>>> 54a170d19337de0474ec43d922d34467dcb997fc:molGCT_sampling.py
