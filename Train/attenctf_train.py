...
from Utils.dataset import get_dataloader, get_loader
...
def loss_function(preds_prop, preds_mol, ys_cond,
                  ys_mol, use_cond2dec, pad_id):
    ...
    if use_cond2dec == True:
        ...
        loss = RCE_mol + RCE_prop
    else:
        RCE_prop = torch.zeros(1)
        loss = RCE_mol
    return loss, RCE_mol, RCE_prop
...
def run_epoch(args, model, optimizer, dataloader,
              current_step, nbatches, LOG, train):
    n_samples = 0
    n_printevery = 1
    ...
    for i, batch in enumerate(dataloader):
        ...
        preds_prop, preds_mol = model.forward(
            src=batch.src,
            trg=trg_input,
            econds=batch.econds,
            mconds=batch.mconds,
        ...
        if train:
            optimizer.zero_grad()
        loss, RCE_mol, RCE_prop = loss_function(preds_prop,
                                                preds_mol,
                                                ys_cond,
                                                ys_mol,
                                                args.use_cond2dec,
    ...
    if train: # training phase
        return history, avg_rce, current_step
    else: # validation phase
        return history, avg_rce
...
def train_model(args, model, optimizer, train_iter,
    ...
    for epoch in range(args.start_epoch, args.num_epoch+1):
        ...
        dataloader = get_loader(train_iter, args.conditions,
            args.pad_id, args.max_strlen, args.pad_to_same_len, device)
        ...
        LOG.info(f'training end\tRCE: {avg_loss}')
        ...
        LOG.info(f'validation end\tRCE: {avg_loss}')
...
def attenctf_train(args, logger):
    ...
    train, valid = data.TabularDataset.splits(
        path=data_path, train='train_half.csv', validation='valid_half.csv', # change
    ...
    if args.use_model_path is not None:
