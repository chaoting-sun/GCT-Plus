...
class ATT_Trainer(object):
    ...
    def get_optimization(self, trainable_parameters, opt_name):
        """
    ...
    def save_checkpoint(self, model, opt_mu, opt_logvar,
                        model_name, src_vocab_size, trg_vocab_size):
    ...
    def run_epoch(self, model, data_iter, nbatches, TRG,
        ...
        def compute_loss(pred, truth):
            return MSE(pred, truth), KLDiv(pred, truth)
    ...
    def train(self, model, train_iter, valid_iter, SRC, TRG, device):
        ...
        opt_mu = self.get_optimization(filter(lambda p: p.requires_grad, 
                                       model.parameters()), 'state_dict_mu')
        opt_logvar = self.get_optimization(filter(lambda p: p.requires_grad,
        ...
        for epoch in range(self.args.start_epoch, self.args.num_epoch+1):
            ...
            if lowest_kldiv > (valid_mu_kld*valid_logvar_kld)**0.5:
                ...
                self.save_checkpoint(model, opt_mu, opt_logvar, f"best_{epoch}.pt",
            ...
            self.save_checkpoint(model, opt_mu, opt_logvar, f"model_{epoch}.pt",
                                 len(SRC.vocab), len(TRG.vocab))
