...
class Sampler(nn.Module):
    ...
    def conditional_sampling(self, mu, logvar, threshold=10):
        std = torch.exp(0.5*logvar)
        eps = torch.randn_like(std)
        z = eps.mul(std).add(mu)
        return torch.where(std > threshold, mu, z) # 要 return sample 出來的東西
...
class Encoder(nn.Module):
    ...
    def forward(self, src, conds, mask):
        ...
        mu = self.fc_mu(x)
        log_var = self.fc_log_var(x)
        return self.sampler.conditional_sampling(mu, log_var), mu, log_var
...
class CVAETFCUT(nn.Module):
    ...
    def forward(self, src, trg, econds, dconds, src_mask, trg_mask):
        z, mu, log_var = self.encode(src, econds, src_mask)
