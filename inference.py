...
def inference(opt, model, SRC, TRG):
    ...
    if opt.conds == 'm':
        ...
        toklen_data = tokenlen_gen_from_data_distribution(data=toklen_data, size=n_samples, nBins=int(toklen_data.max()-toklen_data.min()))

        start = time.time()
        for idx in range(n_samples):
            toklen = np.int32(toklen_data[idx]) + 3  # +3 due to cond2enc
            z = torch.Tensor(np.random.normal(size=(1, toklen, opt.latent_dim)))
            molecule_tmp = gen_mol(conds[idx], model, opt, SRC, TRG, toklen, z) 
            # print("molecule_tmp:", molecule_tmp)
            ...
            if (idx+1) % 2000 == 0:
                ...
                gen_list = pd.DataFrame({"mol": molecules, "val_check": val_check, 
                                         "trg(logP)": np_conds_trg[:, 0], 
                                         "trg(tPSA)": np_conds_trg[:, 1], 
                                         "trg(QED)": np_conds_trg[:, 2], 
                                         "rdkit(logP)": np_conds_rdkit[:, 0], 
                                         "rdkit(tPSA)": np_conds_rdkit[:, 1], 
                                         "rdkit(QED)": np_conds_rdkit[:, 2], 
                                         "toklen": toklen_check, 
...
def main():
    ...
    parser.add_argument('-load_toklendata', type=str, default='data/moses/toklen_list.csv')
    ...
    while True:
        ...
        \n* Typing sample: 2.2, 85.3, 0.8\n* Enter the properties (Or type m for MOSES benchmark, q: quit):".format(opt.min_logP, opt.max_logP, opt.min_tPSA, opt.max_tPSA, opt.min_QED, opt.max_QED))

        if opt.conds == "m": # for MOSES benchmark
            molecule = inference(opt, model, SRC, TRG)
            break
        if opt.conds == "q": # quit the program
            break
        else:
            molecule = inference(opt, model, SRC, TRG)
