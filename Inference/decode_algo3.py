...
def generate_latent_space(toklen_data, latent_dim, device):
    ...
    z = torch.Tensor(np.random.normal(size=(1, toklen, latent_dim)))
...
class Sampling(object):
    ...
    def random_z(self, n=1):
        return generate_latent_space(self.toklen_data,
                                     self.latent_dim,
                                     self.device, n)
        
    def random_z_with_toklen(self, toklen, n=1):
...
class BeamSearchFromSource(BeamSearch):
    ...
    def sample_smiles(self, src, conds, std=0.2, transform=True):
        ...
        src_mask = create_source_mask(src, self.pad_idx, econds)
        z = self.predictor.encode(src, econds, src_mask)[0]
