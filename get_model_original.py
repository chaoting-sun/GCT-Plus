...
def get_model(opt, SRC, TRG):
    model = transformer.Transformer(len(SRC.vocab), len(TRG.vocab), opt.N, opt.d_model, 
                                    opt.d_ff, opt.H, opt.latent_dim, opt.dropout,
                                    opt.nconds, use_cond2dec=False, use_cond2lat=True)
    model.load_state_dict(torch.load(opt.molgct_path))
...
def inference():
    ...
    opt.device = 0
    opt.k = 4
    opt.molgct_path = 'molGCT/molgct.pt'
