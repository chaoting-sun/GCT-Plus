...
class EvaluationRunner:
    ...
    def LogP_stat(self):
        ...
        plus_minus_change = [cfgd.PROPERTY_ERROR['LogP']]
        ...
        for flu in plus_minus_change:  # for each fluctuation
            ...
            for i in range(self.num_samples):
                pred_LogP = self.data['Predict_smi_{}_c{}'.format(i + 1, property_name)].tolist()
                zipped = list(zip(pred_LogP, source_property, delta_property, range_list, range_evaluation_pool))
        ...
        for i, flu in enumerate(plus_minus_change):
            ...
            up.hist(self.data, label, name=label, path=self.output_path,
                    title=r"Number of molecules with LogP in desired$\pm${}".format(flu))
        ...
        for i, flu in enumerate(plus_minus_change):
            ...
            up.hist_box(self.data, label, name=label + "_hist_box",
                        ...
                        title=r"Number of molecules with desired LogP $\pm${}".format(flu))
            LOG.info(self.data[label].describe())


    def LogP_stat_prep(self):
        ...
        # best delta LogP from generated molecules
    ...
    def compute_similarity(self):
        ...
        similarities = []
        for i in range(self.num_samples):
            pred_smi_list = self.data[f'Predicted_smi_{i+1}'].tolist()
            zipped = list(zip(source_smiles_list, pred_smi_list)) # [(source smi1, target smi1), (source smi2, target smi2),...]
            with Pool(NUM_WORKERS) as p:
...
def is_inrange(pred_LogP, source_LogP, delta_LogP, range, range_evaluation):
    if range_evaluation == "":
        return 1 if abs(pred_LogP - (source_LogP + delta_LogP)) < range else 0
    elif range_evaluation == "lower":
        return 1 if pred_LogP >= cfgd.LOD_MIN and pred_LogP < min(source_LogP + range, cfgd.LOD_MAX) else 0
    elif range_evaluation == "higher":
        return 1 if pred_LogP > max(source_LogP - range, cfgd.LOD_MIN) and pred_LogP <= cfgd.LOD_MAX else 0
    else:
