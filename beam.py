...
Changes:
1) Function Argument
:from: init_vars(cond, model, SRC, TRG, toklen, opt, z)
: to : init_vars(cond, model, toklen, opt, z)
2) init_tok
:from: init_tok = TRG.vocab.stoi['<sos>']
: to : init_tok = 1 # 1 is the start token.
"""
def init_vars(cond, model, toklen, opt, z):
...
"""REF: molGCT/Beam.py
1) Function Argument
:from: beam_search(cond, model, SRC, TRG, toklen, opt, z)
: to : beam_search(cond, model, SRC, TRG, toklen, opt, z)
2)
:from:
: to :
3)
...
def beam_search(cond, model, SRC, TRG, toklen, opt, z):
    ...
    outputs, e_outputs, log_scores = init_vars(cond, model, toklen, opt, z)
