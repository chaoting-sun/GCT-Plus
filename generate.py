...
def _unique(smiles): return len(np.unique(smiles)) / \
    len(smiles) * 100 if len(smiles) else 0


def _intdiv(valid_smiles): return metrics.internal_diversity(
    valid_smiles) if len(valid_smiles) else 0
...
def generate_metrics_line(preds):
    ...
    head = 'validity\tuniqueness\tdiversity\t'        \
           'logp_mae\tlogp_mse\tlogp_max\tlogp_min\t' \
           'tpsa_mae\ttpsa_mse\ttpsa_max\ttpsa_min\t' \
           'qed_mae\tqed_mse\tqed_max\tqed_min'
...
def predict_molecules(args, model, target_properties, TRG, scaler, toklen_data,
    ...
    for logp, tpsa, qed in target_properties:
        ...
        with open(os.path.join(args.storage_path,
                               f'{logp:.2f}_{tpsa:.2f}_{qed:.2f}.txt'), 'w', buffering=10) as sample_file:
...
if __name__ == "__main__":
    ...
    if args.demo:
        ...
        target_properties = np.array(np.meshgrid(np.linspace(args.logp_lb, args.logp_ub, num=num_points),
                                                 np.linspace(
                                                     args.tpsa_lb, args.tpsa_ub, num=num_points),
        ...
        for logp, tpsa, qed in target_properties:
            ...
            with open(os.path.join(args.storage_path,
                      f'{logp:.2f}_{tpsa:.2f}_{qed:.2f}_mean.txt'), 'w') as metrics_writer:
